...
{
    [TestFixture]
    public class DefaultListModelTest : IItemModelTest
    {
        DefaultListModel<TestType> model;
        [SetUp]
        public void Setup()
        {
            model = new DefaultListModel<TestType>();
            testModel = model;
        }
        ...
        public void SetDataTest()
        {
            ...
            Assert.AreEqual(2, model.Rows);
            Assert.AreEqual(1, model.Columns);
            Assert.AreEqual(t2, model.DataAt(1, 0));

            Assert.IsTrue(model.SetData(t2, 2, 2));
            Assert.AreEqual(3, model.Rows);
            Assert.AreEqual(1, model.Columns);
            Assert.AreEqual(t2, model.DataAt(2, 0));
