...
namespace BattleMode.Shared
{
    public class Client
    {
        private readonly List<Pokemon.Models.Pokemon> _pokemons = new List<Pokemon.Models.Pokemon>();
        private readonly List<Item> _items = new List<Item>();

        public Client(ClientIdentifier id, List<Pokemon.Models.Pokemon> pokemons)
        {
            Id = id;
        ...
        public Client(ClientIdentifier id)
        {
            ...
            for (var i = 0; i < 6; i++)
            {
                var pkmn = new Pokemon.Models.Pokemon(data, stats) { Name = Id.Name + "_Pkmn" + i, Level = i + 20};
                for (var j = 0; j < 2; j++)
        ...
        public IReadOnlyList<Pokemon.Models.Pokemon> Pokemons => _pokemons.AsReadOnly();
    }
}