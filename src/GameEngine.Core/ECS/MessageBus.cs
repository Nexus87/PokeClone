...
namespace GameEngine.Core.ECS
{
    public class MessageBus : IMessageBus
    {
        ...
        {
            if (_handlerMap.ContainsKey(handler)) return;

            void Wrapper(object x, IEntityManager y, IMessageBus z)
            {
                handler((TAction) x, y, z);
            }
        ...
        public void UnregisterHandler<TAction>(Action<TAction, IEntityManager, IMessageBus> handler)
        {
            UnregisterHandler<TAction>((object) handler);
        }

        public void UnregisterHandler<TAction>(Action<IEntityManager, IMessageBus> handler)
        {
            UnregisterHandler<TAction>((object) handler);
        }

        public void UnregisterHandler<TAction>(Action<TAction, IMessageBus> handler)
        {
            UnregisterHandler<TAction>((object) handler);
        }

        public void UnregisterHandler<TAction>(Action<IMessageBus> handler)
        {
            UnregisterHandler<TAction>((object) handler);
        }

        public void UnregisterHandler<TAction>(Action handler)
        {
            UnregisterHandler<TAction>((object) handler);
        }
        ...
        public void StartProcess()
        {
            while (_messageQueue.Count > 0)
            {
                ...
                if (action.GetType() != typeof(TimeAction))
                    Console.WriteLine($"Processing Action of Type: {action.GetType().Name}");
        ...
        {
            if (typeof(TAction) != typeof(TimeAction))
                Console.WriteLine($"{sender?.GetType().Name} sending action of type {action.GetType().Name}");
        ...
        {
            if (!_handlerMap.TryGetValue(handler, out var wrapper)) return;
